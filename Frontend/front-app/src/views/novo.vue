<template>
    ...
    methods: {
      ...
      addFactory() {
        if (this.validateWorkingHours()) {
          axios
            .post('http://localhost:8080/WebShopAppREST/rest/factories/addFactory', this.newFactory)
            .then((response) => {
              if (this.newFactory.selectedManager) {
                this.assignManager(response.data.id, this.newFactory.selectedManager);
              } else {
                alert('Factory added successfully');
                this.$store.commit('resetFactoryForm'); // Reset the form
      ...
      assignManager(factoryId, managerUsername) {
        axios
          .put(http://localhost:8080/WebShopAppREST/rest/users/${managerUsername}/assignFactory/${factoryId})
          .then((response) => {
  ...
  i evo index.js da vidis putanju do signUpManager:{
      path: '/sign-up-manager/:factoryId',
      name: 'SignUpManager',
